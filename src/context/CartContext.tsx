...
interface CartContextType {
  ...
  removeAllFromCart: (selectedItems: string[]) => Promise<void>
}
...
export const CartProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const saveToCheckOut = async (paymentMethod?: string) => {
    ...
    try {
      const selectedProducts = cartItems.cartItem.filter((item) => selectedItems.includes(item.id) && item.quantity > 0)
      ...
      const existingUserCheckOut = response.data.find((item: any) => item.userId === currentAccount.id)

      if (existingUserCheckOut) {
        const updatedCartItems = [...existingUserCheckOut.cartItems, newCartItem]
      ...
      const remainingItems = cartItems.cartItem.filter((item) => !selectedItems.includes(item.id))
  ...
  const removeAllFromCart = async (selectedItems: string[]) => {
    if (!cartItems || !cartItems.id) {
      console.error('Cart is empty or cart ID is missing.')
      return
    }
    try {
      // Lọc các mục không nằm trong `selectedItems`
      const updatedCartItems = cartItems.cartItem.filter((item) => !selectedItems.includes(item.id))

      // Gửi yêu cầu cập nhật giỏ hàng trên server
      await axios.patch(`${API_ROOT}/ownCart/${cartItems.id}`, { cartItem: updatedCartItems })

      // Cập nhật trạng thái giỏ hàng trên client
